...
function test_From_ClipBoard_and_Put_Back(): string
{
    ...
    $time = "echo 'time passed = ' . number_format(microtime(true) - \$start,  6, '.', ' ') . PHP_EOL;";
...
function clipBoard_Get(): string
{
    if (PHP_OS_FAMILY === 'Windows') {
        ...
        return substr(
            shell_exec('powershell -sta "add-type -as System.Windows.Forms; [windows.forms.clipboard]::GetText()"'),
            0,
            -1
        );
        ...
        return substr(
            shell_exec('xclip -out -selection primary'),
            0,
            -1
        );
...
function implode_recursive(
    ...
    // Trim ALL whitespace
    $trim_all and $glued_string = preg_replace("/(\s)/ixm", '', $glued_string);
